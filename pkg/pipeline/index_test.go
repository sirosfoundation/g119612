...
func TestGenerateIndex(t *testing.T) {
	...
	t.Run("Basic Index Generation", func(t *testing.T) {
		...
		// Check if report.html was also created (default behavior)
		reportPath := filepath.Join(htmlDir, "report.html")
		_, err = os.Stat(reportPath)
		assert.NoError(t, err, "report.html should exist by default")
		...
		// Cleanup report for other tests
	...
	t.Run("Report Generated By Default", func(t *testing.T) {
		ctx := NewContext()
		ctx.EnsureReport()
		ctx.Report.AddIssue(SeverityWarning, "test", "test-source", "test warning")
		ctx.Report.RecordStep("load")
		ctx.Report.RecordStep("select")

		// Call generate_index - should generate report.html by default
		_, err := GenerateIndex(nil, ctx, htmlDir, "Test Index")
		assert.NoError(t, err)
		...
		assert.Contains(t, string(content), "Pipeline Report")

		// Clean up
		os.Remove(reportPath)
	})

	t.Run("No Report Flag", func(t *testing.T) {
		ctx := NewContext()
		ctx.EnsureReport()
		ctx.Report.AddIssue(SeverityInfo, "test", "test-source", "test info")

		// Call with no-report flag
		_, err := GenerateIndex(nil, ctx, htmlDir, "Test Index", "no-report")
		assert.NoError(t, err)

		// Check that report.html was NOT created
		reportPath := filepath.Join(htmlDir, "report.html")
		_, err = os.Stat(reportPath)
		assert.True(t, os.IsNotExist(err), "report.html should not exist with no-report flag")
	})
}
