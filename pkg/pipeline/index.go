...
// By default, a report.html file is also generated with pipeline execution statistics.
//
// Arguments:
//   - arg[0]: Directory path containing TSL HTML files
//   - arg[1]: (Optional) Title for the index page (default: "Trust Service Lists Index")
//   - arg[N]: (Optional) "no-report" to skip report generation
//
...
func GenerateIndex(pl *Pipeline, ctx *Context, args ...string) (*Context, error) {
	...
	for i, arg := range args {
		if i == 0 {
			continue // skip dirPath
		}
		if arg == "no-report" {
			generateReport = false
		} else if !strings.HasPrefix(arg, "no-") {
			// Assume it's the title if not a flag
			title = arg
		}
	}
	...
	// Generate the report.html file first (so index.html can link to it)
	if generateReport {
		ctx.EnsureReport() // make sure report exists
		reportPath := filepath.Join(dirPath, "report.html")
		reportTitle := title + " - Pipeline Report"
		if err := RenderReportToFile(ctx, reportPath, reportTitle, pl); err != nil {
			// Log warning but don't fail the step
			if pl != nil && pl.Logger != nil {
				pl.Logger.Warn("Failed to generate report",
...
func generateIndexHTML(dirPath string, entries []TSLIndexEntry, title string, report *PipelineReport) error {
	...
	if report != nil {
		// Check if a report.html exists next to the index (generated by us or separately).
